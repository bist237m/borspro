...
class BorsaAPI {
    ...
    async getIndex(symbol = 'XU100') {
        try {
            ...
            if (this.useMockData) {
                ...
                const response = await axios_1.default.get(`${this.baseURL}/embed/borsa.json`, {
    ...
    async getStock(symbol) {
        try {
            ...
            if (this.useMockData) {
                const mockData = this._generateMockData();
                const stockData = mockData.find(item => item.code === symbol.toUpperCase() || item.code === symbol.toUpperCase() + '.E');
                if (!stockData) {
                ...
                const stockData = response.data.find((item) => item.code === symbol || item.code === symbol + '.E');
                if (!stockData) {
                    throw new Error(`${symbol} hissesi bulunamadı`);
                }
                data = stockData;
    ...
    async getAllIndexes() {
        try {
            ...
            if (this.useMockData) {
                ...
                const results = await Promise.all(promises);
                return results.filter((idx) => idx !== null);
            }
            else {
                const response = await axios_1.default.get(`${this.baseURL}/embed/borsa.json`, {
                    timeout: this.timeout,
    ...
    async searchStock(query) {
        try {
            ...
            if (this.useMockData) {
                ...
                results = mockData.filter(item => (item.code && this._normalizeTurkish(item.code).includes(searchTerm)) ||
                    (item.name && this._normalizeTurkish(item.name).includes(searchTerm)));
            }
            else if (this.useRealData) {
                const popularStocks = await this.getPopularStocks();
                const searchTerm = this._normalizeTurkish(query);
                return popularStocks.filter(item => (item.symbol && this._normalizeTurkish(item.symbol).includes(searchTerm)) ||
                    (item.name && this._normalizeTurkish(item.name).includes(searchTerm)));
            }
            else {
                const response = await axios_1.default.get(`${this.baseURL}/embed/borsa.json`, {
                ...
                const searchTerm = this._normalizeTurkish(query);
                results = response.data.filter((item) => (item.code && this._normalizeTurkish(item.code).includes(searchTerm)) ||
                    (item.name && this._normalizeTurkish(item.name).includes(searchTerm)));
            }
    ...
    async getHistoricalData(symbol, options = {}) {
        try {
            ...
            const { period = '1mo', interval = '1d', period1, period2 } = options;
