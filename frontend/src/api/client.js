...
export const stocks     = { list: (p={}) => request("GET",`/stocks${p&&Object.keys(p).length?"?"+new URLSearchParams(p):""}`), get: s => request("GET",`/stocks/${s}`), history: (s,p) => request("GET",`/stocks/${s}/history?period=${p}`), filters: s => request("GET",`/stocks/${s}/filters`), fundamentals: s => request("GET",`/stocks/${s}/fundamentals`), news: s => request("GET",`/stocks/${s}/news`), extraFilters: s => request("GET",`/stocks/${s}/extra-filters`) };
